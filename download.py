...
#		import pdb; pdb.set_trace()
	...
	def dataframe(self):
		if len(self.prices) == 0:
			self.load_csv()
		dateData = pd.read_csv(self.filePath)
#		import pdb; pdb.set_trace()
#		print(date[0:1])

		#最初の日付のときの日付のフォーマット確認
		if dateData['Date'][len(dateData)-1].find(self.DateSp) != -1: #'/'を含む場合，文字列変換
			dateData = dateData.replace({self.DateSp: self.DateSpNew}, regex=True)
		#降順にソート
		dateData = dateData.sort(ascending='False')
		#Dateをインデックスにして返す
#		import pdb; pdb.set_trace()
		return dateData.set_index('Date')
	...
	@classmethod
	def _str_to_date(cls, str_date, str_ChaIf):
		dateSpNew = '-'
		'日付の区切りを / から - に変更'
#		import pdb; pdb.set_trace()
		date = str_date.replace(str_ChaIf,dateSpNew) #最新の日付の読み込み
		date = date.split(dateSpNew)
...
#		html = urllib2.urlopen(self._url(page)).read()
		html = urllib.request.urlopen(self._url(page)).read().decode('utf-8')
			...
			if len(pagePrices) > 0:
				if limit_date:
					for price in pagePrices:
						date = self._str_to_date(price[0], self.DateSp)
						if date > limit_date:
							prices.append(price)
						else:
							fetch = False
...
#		import pdb; pdb.set_trace()
		if len(prices) >= 2:
			last_date = self._str_to_date(prices[1][0], self.DateSp)
